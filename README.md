# OkadaField 0.4.2 beta

**対応環境：Windows 64-bit (x64) のみ**  
**表示言語：日本語のみ**

> 現在の beta 版は Windows 64-bit 専用・日本語 UI のみです。macOS，Linux，32-bit Windows，英語 UI には対応していません。
...
> **Beta software**：研究・教育用途での試用とフィードバック収集を想定したベータ版です。重要な解析では，入力値・符号規約・単位を確認し，必要に応じて独立な計算との照合を行ってください。

## 主な機能

- 1 枚の有限矩形ソース
- 任意断層，逆断層例，正断層例，横ずれ断層例，ダイク，シルのプリセット
- 任意の strike / dip / length / width / strike-slip / dip-slip / opening
- 水平面（地表または任意深度）の分布
- East–Depth / North–Depth の鉛直断面（ローカル座標モード）
...
- 剛性率 μ を入力し，Poisson 比 ν と合わせて第1 Lamé 定数 λ を自動計算
- receiver strike / dip / rake と有効摩擦係数 μ′ を指定可能
- 「受け手断層をソース断層に合わせる」で source strike / dip / rake を自動利用
- 傾斜：∂Uup/∂E, ∂Uup/∂N
- 変位ベクトル重ね描き
- プロット領域の距離スケールを縦横等倍（1:1）で表示
- 緯度経度入力と地理院地図上への重ね描き
- 計算範囲の自動設定：基準点から ±5 km / ±50 km / ±500 km
- 背景地図を計算範囲の縦横約 2 倍へ自動スケール
- PNG / CSV 出力
- JSON 設定ファイルの保存・読込

## 0.4.2 beta の主な変更点

- **プロット領域の縦横距離スケールを等倍（1:1）**にしました。
  - 水平面では East–North の 1 km を縦横で同じ画面長として表示します。
  - East–Depth / North–Depth 断面でも，水平距離 1 km と深さ 1 km を同じ画面長として表示します。
  - 緯度経度モードでは，背景地図の Web Mercator 投影上で縦横の局所スケールが一致するよう描画領域を調整します。
- これにより，変位ベクトルの向きと相対的な長さを，図の縦横比による歪みなしで比較しやすくしました。
- 数値計算，歪み・応力・Coulomb 応力計算には変更ありません。
- README の地理院地図に関する説明を利用者向けに簡潔化しました。

## 0.4.1 beta の主な変更点

- **応力系のカラースケール設定**を追加しました。
  - `共通（応力6成分 / Coulomb 3量）`：**既定値**。応力テンソル6成分は6成分間で，`Δτ`・`Δσn`・`ΔCFF` は3量間で同じ正負対称スケールを用います。
  - `自動（表示量ごと）`：従来どおり，現在の表示量の最大絶対値に合わせます。
  - `範囲指定（± MPa）`：ユーザーが正負対称の表示範囲を直接指定できます。
- これにより，`Δτ` と `ΔCFF` の空間パターンがよく似ていても，振幅差を同じカラースケールで直接比較できます。
- カラースケール設定は JSON 設定ファイルにも保存されます。

## 0.4.0 beta の主な変更点

- **応力変化表示**を追加しました。
  - ΔσEE, ΔσNN, ΔσUU, ΔσEN, ΔσEU, ΔσNU
  - 表示・CSV 出力単位は MPa
- **Coulomb 応力変化**を追加しました。
  - Δτ：receiver の rake 方向を正
  - Δσn：引張 / unclamping を正
  - ΔCFF = Δτ + μ′Δσn
- 弾性定数として **剛性率 μ (GPa)** を追加しました。第1 Lamé 定数 λ は Poisson 比 ν と μ から自動計算します。
- receiver fault の strike / dip / rake と **有効摩擦係数 μ′** を入力できます。
- `受け手断層をソース断層に合わせる` を ON にすると，receiver strike / dip は source と同じ値，rake は `atan2(DISL2, DISL1)` から自動設定します。
- opening のみで `DISL1=DISL2=0` の場合は source rake が定義できないため，ΔCFF を評価するときは receiver を個別指定することを推奨します。
- v0.3.2 で整理した簡潔な断層投影（破線 + 基準点）と大きめの軸ラベルは維持しています。
- 公開パッケージには GUI 版 `OkadaField.exe` を収録し，ソースコードは本配布物に含めていません。
...
- 表示傾斜：µrad
- `z` は上向きを正とする DC3D の規約に合わせ，観測深さ `d` km では `z=-d` としています。
- strike は北から時計回りです。
- 右手系 (RHR) として dip direction = strike + 90° としています。
...
局所〜地域スケールの変形可視化を想定した近似です。±500 km のような広域では誤差が増えるため，定量解析では測地線計算や ECEF/ENU 変換の利用を検討してください。

緯度経度モードは現在 **水平面表示**に対応しています。鉛直断面はローカル座標モードで使用してください。
...
「設定を保存 (.json)」では，以下を JSON に保存します。

- ソースパラメータ
- Poisson 比 ν，剛性率 μ
- receiver strike / dip / rake，有効摩擦係数 μ′
- ローカル / 緯度経度モード
- 計算範囲と grid size
- 観測深さ / 断面設定
- 表示量
...
「設定を読込」で同ファイルを選ぶと，値を復元した後に自動計算します。

`examples/normal_fault_geo.json` に緯度経度モードの正断層例を同梱しています。

## 数値計算と検証

本公開パッケージは実行形式のみを配布します。数値計算は Okada (1992) に基づく DC3D 有限矩形ソース解をベースとしています。元の DC3D Fortran コードおよびマニュアルは，防災科研の公式公開ページを参照してください。
...
v0.4.0 ではさらに，(1) `ν=0.25, μ=30 GPa` で `λ=30 GPa` となること，(2) receiver 面へ与えた既知の normal/shear stress が Δσn, Δτ, ΔCFF に正しい符号で分解されること，(3) 地表 `z=0` で自由表面の traction 成分 ΔσUU, ΔσEU, ΔσNU が数値誤差範囲で 0 になることをテストしています。
